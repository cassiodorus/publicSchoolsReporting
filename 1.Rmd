---
title: "publicSchoolsGLPCCR Project - Read Files"
author: "D. Hopp"
date: "June 2017"
output:
...
```{r omment,eval=FALSE,echo=FALSE}
# force exit
if (any(thisname,ls())) {
	print(paste("Already there:",thisname))
	knitr::knit_exit() 
}
```  
	...
	fcnGetZippedLEAdata6clean <- function(L) {
		...
		df_ <- L %>%
			dplyr::filter(  trimws(subject) %in% c("ALL","EOG",forSubjects) & trimws(type)=="ALL" & 
							trimws(subgroup) %in% c("ALL","AIG","BLCK","EDS","HISP","NOT_EDS","WHTE") & nchar(trimws(school_code))==6 & 
...
dfDisag_YrA <- fcnGetZippedLEAdata8( "disag1314.zip","Disag_2013-14/Disag_2013-14_Data.txt")[["All"]] %>% 
		...
		dplyr::mutate( 	num_3=num_glp-num_ccr,
...
#
# grades 3, 4, 5
dfDisagEx20_345_YrA <- dfDisagEx20_YrA %>%
		dplyr::filter( 
			grade %in% c("03","04","05") & 
			subject %in% forSubjects 
		) 
str(dfDisagEx20_345_YrA)
#
```  
...
str(dfDisag_YrB)
#
dfDisagEx20_YrB <- dfDisag_YrB %>%
		dplyr::anti_join(dfAlternativesYrC, by="school_code") %>%
		dplyr::filter( num_tested >= 20 )
str(dfDisagEx20_YrB)
#
dfDisagEx20_345_YrB <- dfDisagEx20_YrB %>%
		dplyr::filter( grade %in% c("03","04","05") & 
				subject %in% forSubjects ) 
str(dfDisagEx20_345_YrB)
#
```  
...
str(dfDisagEx20_YrC)
#
dfDisagEx20_345_YrC <- dfDisagEx20_YrC %>%
		dplyr::filter( 
			grade %in% c("03","04","05") & 
			subject %in% forSubjects 
		) 
str(dfDisagEx20_345_YrC)
#
```  
...
#
# grades 3, 4, 5
dfDisagEx20_345_YrD <- dfDisagEx20_YrD %>%
		dplyr::filter( 
			grade %in% c("03","04","05") & 
			subject %in% forSubjects 
		) 
str(dfDisagEx20_345_YrD)
#
# #######################################################################
```
...
#
# keep this
dfDisagEx20_345_Yr <- rbind(
				dfDisagEx20_345_YrA,
				dfDisagEx20_345_YrB,
				dfDisagEx20_345_YrC,
				dfDisagEx20_345_YrD
			)
str(dfDisagEx20_345_Yr)
#
save(dfDisagEx20_345_YrA,file=paste0(RDataPath,"dfDisagEx20_345_YrA.RData"))
save(dfDisagEx20_345_YrB,file=paste0(RDataPath,"dfDisagEx20_345_YrB.RData"))
save(dfDisagEx20_345_YrC,file=paste0(RDataPath,"dfDisagEx20_345_YrC.RData"))
save(dfDisagEx20_345_YrD,file=paste0(RDataPath,"dfDisagEx20_345_YrD.RData"))
save(dfDisagEx20_345_Yr,file=paste0(RDataPath,"dfDisagEx20_345_Yr.RData"))
#
# keep this
dfDisagEx20_Yr <- rbind(
				dfDisagEx20_YrA,
#				dfDisagEx20_345_YrB, <- error? changed 9/7/17
				dfDisagEx20_YrB,
				dfDisagEx20_YrC,
				dfDisagEx20_YrD
			)
str(dfDisagEx20_Yr)
#
save(dfDisagEx20_YrA,file=paste0(RDataPath,"dfDisagEx20_YrA.RData"))
save(dfDisagEx20_YrB,file=paste0(RDataPath,"dfDisagEx20_YrB.RData"))
save(dfDisagEx20_YrC,file=paste0(RDataPath,"dfDisagEx20_YrC.RData"))
save(dfDisagEx20_YrD,file=paste0(RDataPath,"dfDisagEx20_YrD.RData"))
save(dfDisagEx20_Yr,file=paste0(RDataPath,"dfDisagEx20_Yr.RData"))
#
# removal to keep house clean
# these can be loaded if necessary
rm(dfDisag_YrA,dfDisag_YrB,dfDisag_YrC,dfDisag_YrD)
rm(dfDisag_Yr)
rm(dfDisagEx20_YrA,dfDisagEx20_YrB,dfDisagEx20_YrC,dfDisagEx20_YrD)
rm(dfDisagEx20_345_YrA,dfDisagEx20_345_YrB,dfDisagEx20_345_YrC,dfDisagEx20_345_YrD)
# ########################################################
```  
...
aggregate( 
	...
	data = dfDisagEx20_345_Yr[dfDisagEx20_345_Yr$subject=="MA" & dfDisagEx20_345_Yr$subgroup=="ALL",], 
	FUN = length 
)
#
# #################################
...
#
dfSchoolEx20_345_allyears <- dfDisagEx20_345_Yr %>% 
...
# silently uses dfDisagEx20_345_Yr
#
