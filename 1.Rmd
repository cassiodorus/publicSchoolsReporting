---
title: "publicSchoolsReporting Project - Read Files"
author: "D. Hopp"
date: "November 2017"
output:
...
inventory <- data.frame(
    "obj_name"=character(0),
    "obj_class"=character(0),
    "obj_purpose"=character(0),
    "obj_uses"=character(0),
    "obj_created_in"=character(0),
    stringsAsFactors=FALSE
  )
# inventory[nrow(inventory) + 1,] = list("v1","v2","v3","v4","v5")
#
# paths
...
#
# added 11/23/17
# clean up District so that it matches Census Bureau school district shapefiles
#
dfLEA %<>%
  dplyr::mutate(
    District=stringr::str_replace(District," Public","")
  )
dfLEA %<>%
  dplyr::mutate(
    District=stringr::str_replace(District,"Newton Conover","Newton-Conover")
  )
dfLEA %<>% 
  dplyr::mutate(
    District=ifelse(
      stringr::str_detect(District,"Schools"),
      District,
      stringr::str_replace(District,"School","Schools")
    )
  )
# ########################################################
#
...
## Combine and winnow data frames  
```{r more_combine}
inventory[nrow(inventory) + 1,] = list("dfDisag_Yr","df","basic disag data","disag files","1")
dfDisag_Yr <- rbind(
  dfDisag_YrA,
  dfDisag_YrB,
  dfDisag_YrC,
...
#
rm(dfDisag_Yr) # no longer used
rm(dfDisag_YrA,dfDisag_YrB,dfDisag_YrC,dfDisag_YrD)
#
inventory[nrow(inventory) + 1,] = list("dfDisagX_Yr","df","disag data disciplinary schools removed","dfDisagX_YrA etc.","1")
dfDisagX_Yr <- rbind(
  dfDisagX_YrA,
  dfDisagX_YrB,
  dfDisagX_YrC,
...
str(dfDisagX_Yr)
#
save(dfDisagX_YrA,file=paste0(RDataPath,"dfDisagX_YrA.RData"))
save(dfDisagX_YrB,file=paste0(RDataPath,"dfDisagX_YrB.RData"))
save(dfDisagX_YrC,file=paste0(RDataPath,"dfDisagX_YrC.RData"))
save(dfDisagX_YrD,file=paste0(RDataPath,"dfDisagX_YrD.RData"))
#
rm(dfDisagX_YrA,dfDisagX_YrB,dfDisagX_YrC,dfDisagX_YrD)
#
inventory[nrow(inventory) + 1,] = list("dfDisagX20_Yr","df","disag data small schools removed","dfDisagX20_YrA etc.","1")
dfDisagX20_Yr <- rbind(
  dfDisagX20_YrA,
  dfDisagX20_YrB,
  dfDisagX20_YrC,
...
# grades 3, 4, 5
#
inventory[nrow(inventory) + 1,] = list("dfDisagX_345_Yr","df","disag 345 MA RD","dfDisagX_Yr","1")
dfDisagX_345_Yr <-
  dfDisagX_Yr %>%
		dplyr::filter( 
			grade %in% c("03","04","05") & 
...
str(dfDisagX_345_Yr)
#
inventory[nrow(inventory) + 1,] = list("dfDisagX20_345_Yr","df","disag 345 MA RD small schools removed","dfDisagX20_Yr","1")
dfDisagX20_345_Yr <- 
  dfDisagX20_Yr %>%
		dplyr::filter( 
			grade %in% c("03","04","05") & 
...
#
inventory[nrow(inventory) + 1,] = list("dfSchoolX20_345_allyears","df","disag 345 MA RD in >2 years, small schools removed","dfDisagX20_345_Yr","1")
...
```{r fn_dfLEAnums}
inventory[nrow(inventory) + 1,] = list("fn_dfLEAnums","fn","add  some possibly useful columns to dfLEA","dfLEA, dfDisagX20_345_Yr","1")
...
inventory[nrow(inventory) + 1,] = list("dfLEA","fn","dfLEA with added columns","dfLEA","1")
dfLEA <- fn_dfLEAnums()
str(dfLEA)
```  

```{r echo=FALSE}
fn_knit_exit("fn_dfDisagX_345_nLEP_D")
fn_knit_exit("dfDisagX_345_nLEP_D")
```

added 11/13/17

```{r fn_dfDisagX_345_nLEP_D}
inventory[nrow(inventory) + 1,] = list("fn_dfDisagX_345_nLEP_D","df","dfDisagX_345_nLEP_D","dfDisagX_345_Yr","3")
fn_dfDisagX_345_nLEP_D <- function() {
  ...
  dplyr::mutate(
    pct_nLEP=round(nLEP*100/(nLEP+nNOT_LEP)),
    pct_nNOT_LEP=round(nNOT_LEP*100/(nLEP+nNOT_LEP))
  )
  return(df_)
}
str(fn_dfDisagX_345_nLEP_D())
dfDisagX_345_nLEP_D <- fn_dfDisagX_345_nLEP_D()
```  

```{r echo=FALSE}
fn_knit_exit("fn_hist_dfDisagX_345_LEP_D")
```

```{r fn_hist_dfDisagX_345_LEP_D}
fn_hist_dfDisagX_345_LEP_D <- function() {
  # simple histograms of LEP% for charter and non-charter schools
  # 2016-17 rollup grades 3,4,5
  # due to the sparsity of schools with LEP% > 0, two plots are needed
  # to show first all the data, and second only non-zero data
  #
  ret <- list()
  iret_ <- 0
  # public non-charter
  # first, all the schools
  # second, schools with pct_LEP>0
  #
  for (cut_ in 0:1) {
    df_ <- dfDisagX_345_nLEP_D %>%
    dplyr::filter(
      pct_nLEP>=cut_ &
    ...
    n_students <- sum(
      dfDisagX_345_nLEP_D %>% 
      dplyr::filter(charter_school==FALSE & nLEP>0) %>% 
      dplyr::select(nLEP)
    )
    #
    iret_ <- iret_+1
    ...
    labs( 
      ...
      title="LEP Percentage Enrollment 2016-17 Grades 3,4,5 Public Non-Charters",
      subtitle=paste(n_schools,"Schools -",n_students,"Students")
    ) +
    theme(   
  ...
  for (cut_ in 0:1) {
    ...
    n_students <- sum(
      dfDisagX_345_nLEP_D %>% 
      dplyr::filter(charter_school==TRUE & nLEP>0) %>% 
      dplyr::select(nLEP)
    )
    #
    iret_ <- iret_+1
    ...
    labs( 
      x="LEP% Students",
      y="Number of Instances",
      title="LEP Percentage Enrollment 2016-17 Grades 3,4,5 Public Charters",
      subtitle=paste(n_schools,"Schools -",n_students,"Students")
    ) +
    theme(
      legend.text=element_text(size=8),
...
```{r FIPS}
# grab FIPS
# note that dfFIPS$FIPS is numeric
inventory[nrow(inventory) + 1,] = list("dfFIPS","df","county FIPS values","FIPS.csv","1")
dfFIPS <- read.csv(paste0(dataPath,"FIPS.csv"),sep=",",stringsAsFactors = FALSE)
dfFIPS %<>% dplyr::mutate(
  County=ifelse(County=="New Hanover","New_Hanover",County)
)
```  

added 12/21/17
```{r fn_capwords}
inventory[nrow(inventory) + 1,] = list("fn_capwords","fn","caps on 1st char","","1")
fn_capwords <- function(s, strict = FALSE) {
  cap <- function(s) 
    paste(
