---
title: "publicSchoolsReporting Project - Some Plotting Functions"
author: "D. Hopp"
date: "July 2017"
output:
...
```{r fn_boxplots_track}
inventory[nrow(inventory) + 1,] = list("fn_boxplots_track","plts","","dfDisagX20_Yr","2")
      ...
      for (S_ in c("MA","RD") ) {
        df3_ <- df2_ %>%
          dplyr::filter(
            ...
            subgroup %in% c("NOT_EDS","EDS","ALL","BLCK","HISP","WHTE")
        ) 
        #
...
```{r echo=FALSE}
fn_knit_exit("fn_EDT3_tails_YrGS")
```  

```{r fn_EDT3_tails_YrGS}
inventory[nrow(inventory) + 1,] = list("fn_EDT3_tails_YrGS","dcast","","dfDisagX_345_Yr","2")
fn_EDT3_tails_YrGS <- function(argYear,argGrade,argSubject){
  # added 11/2/17
  # look at high-EDS and low-NOT_EDS performing EDTier 3 schools
  # for 2016-17 grade 5 Reading
  # produce list of # of schools
  #
  # a bit complicated because cannot use dfLEA$n_D_05 to count schools
  # because that includes charter and traditional together
  #
  if (length(argYear)>1 | !(argYear %in% unique(dfDisagX_345_Yr$f_acYear))) {
    stop("argYear must letter corresponding to a year.")
  }
  if (length(argGrade)>1 ){
    stop("argGrade must one grade only.")
  }
  if (is.numeric(argGrade)) {
    argGrade <- paste0("0",argGrade)
  }
  if (!(argGrade %in% forGrades345)) {
    stop(paste("argGrade must be in",forGrades345))
  }
  if (length(argSubject)>1 | !(argSubject %in% forSubjects)) {
    stop(paste("argSubject must be in", forSubjects))
  }
  #
  df_ <-
  dfDisagX_345_Yr %>% 
  ...
  reshape2::dcast(
    subgroup+charter_school+LEA~.
  ) %>%
  plyr::rename(
    c("."="N")
  )
  #
  # now figure out how many traditional and charters
  #
  df2_ <-
  dfDisagX_345_Yr %>% 
  dplyr::filter(
    LEA %in% unique(df_$LEA) &
    f_acYear==argYear &
    subject==argSubject &
    grade==argGrade
  ) %>%
  ...
  dplyr::inner_join(
    df_,
    df2_,
    by=c("LEA","charter_school")
  ) %>%
  dplyr::mutate(
    Pct=round(N*100/Schools,digits=1),
    Charter=ifelse(charter_school,"Yes","No")   
  ) %>%
  dplyr::select(
    subgroup,
    Charter,
...
```{r fn_boxplots_comp}
inventory[nrow(inventory) + 1,] = list("fn_boxplots_comp","plts","","dfDisagX20_Yr","2")
...
inventory[nrow(inventory) + 1,] = list("fn_boxplots_cohort","plts","","dfDisagX20_Yr","2")
...
inventory[nrow(inventory) + 1,] = list("fn_boxplots_sbs_cohort","plts","side-by-side","dfDisagX20_Yr","2")
...
```{r echo=FALSE}
fn_knit_exit("fn_dfDisagX20_345_comp")
fn_knit_exit("dfDisagX20_345_comp")
```  

```{r fn_dfDisagX20_345_comp}
# #############################
#
...
# keep all four years
# use dfDisagX20_345_Yr
#
inventory[nrow(inventory) + 1,] = list("fn_dfDisagX20_345_comp","df","","dfDisagX20_Yr","2")
fn_dfDisagX20_345_comp <- function() 
{
  dfDisagX20_345_comp <-
  dplyr::inner_join(
    dfDisagX20_345_Yr %>%
  ...
  return(dfDisagX20_345_comp)
}
#
str(fn_dfDisagX20_345_comp())
dfDisagX20_345_comp <- fn_dfDisagX20_345_comp()
```   


```{r echo=FALSE}
fn_knit_exit("fn_dfDisagX20_345_ABCD_track")
fn_knit_exit("dfDisagX20_345_ABCD_track")
```  

```{r fn_dfDisagX20_345_ABCD_track}
#
# ###########################
#
...
#
inventory[nrow(inventory) + 1,] = list("fn_dfDisagX20_345_ABCD_track","df","four years","dfDisagX20_Yr","2")
fn_dfDisagX20_345_ABCD_track <- function() {
  dfDisagX20_345_ABCD_track <-
  dplyr::inner_join(
    dfDisagX20_345_Yr %>%
      dplyr::select(
        LEA,
  ...
  #
  return(dfDisagX20_345_ABCD_track)
}
str(fn_dfDisagX20_345_ABCD_track())
dfDisagX20_345_ABCD_track <- fn_dfDisagX20_345_ABCD_track()
```  

```{r echo=FALSE}
fn_knit_exit("fn_boxplots_Xcharter")
```  
### compare charter and non-charter schools  

```{r fn_boxplots}
inventory[nrow(inventory) + 1,] = list("fn_boxplots_Xcharter","plts","compare charter/non-charter GLP%","dfDisagX20_Yr","2")
fn_boxplots_Xcharter <- function(argYear,argEDTier="ALL") {
  # compare charters and non-charters for subgroups and argYear and argTier
  # compare one grade at a time
  # use dfDisagX20_Yr
  df_ <- 
    dfDisagX20_Yr %>%
    dplyr::filter(
  ...
  iret_ <- 0
  #
  for (G_ in forGrades345) {
    for (S_ in forSubjects ) {
      df2_ <-
        df_ %>%
        dplyr::filter(
          subject == S_ &
      ...
      dplyr::summarise(
        tot_tested=sum(num_tested),
        tot_glp=sum(num_glp)
      ) %>%
      dplyr::ungroup() %>%
      dplyr::mutate(
        pct_tot_glp=round(tot_glp*100/tot_tested),
        charter=ifelse(charter_school,"Yes","No"),
        Group=subgroup
      ) %>%
      dplyr::select(
      ...
      #      
      n_schoolsP <- nrow(dplyr::distinct(df2_ %>% dplyr::filter(charter=="No") %>% dplyr::select(school_code)))
      n_schoolsC <- nrow(dplyr::distinct(df2_ %>% dplyr::filter(charter=="Yes") %>% dplyr::select(school_code)))
      #
      iret_ <- iret_+1
      ret[[iret_]] <-
      df2_ %>%
      ...
      geom_boxplot() +
      labs( 
        title=paste(names(acYearToLETTER[which(LETTERS==argYear)]),"Public and Charter School GLP% by Group for", lstSubjLong[[S_]],"Grade",G_),
        subtitle=paste(
          "EDTier",EDTierTxt,
          "   ",n_schoolsP,"Public Schools","   ",n_schoolsC,"Charter Schools"
        )
      ) +
      labs( x="Group", y="GLP%" ) +
      ylim( 0, 100 ) +
...
### scatterplot functions  
 ...
 use dfDisagX20_345_track and dfDisagX20_345_comp   
...
inventory[nrow(inventory) + 1,] = list("fn_scatterplots_comp","plts","","dfDisagX20_345_comp","2")
...
#
  ...
  grob <- grid::grobTree(
      textGrob("Filled circles are schools with GLP% under 20% or over 80%.",
      x=0.1,
      y=0.98,
      hjust=-0.45,
      gp=gpar(fontsize=6)
...
inventory[nrow(inventory) + 1,] = list("fn_scatterplots_track","plts","","dfDisagX20_345_track","2")
fn_scatterplots_track <- function(
  argEDTier="ALL",
  argLEAs="ALL",
  argCharterOnly=FALSE,
  argSave=TRUE ) {
# uses dfDisagX20_345_ABCD_track
#
# argEDTier is ALL or in 1:3 and links to dfLEA
# at most one of 1:3
#
...
ditto grade 4 and grade 5  
```{r echo=FALSE}
fn_knit_exit("fn_scatterplots_plotly_comp")
```  

```{r fn_scatterplots_plotly_comp}  
inventory[nrow(inventory) + 1,] = list("fn_scatterplots_plotly_comp","plts","","dfDisagX20_345_comp","2")
    ...
    for ( LEA_ in argLEAs ) {
      ...
      for (S_ in c("MA","RD")) {
        for (G_ in c("03","04","05")) {
          ...
          cmx <- c(cm[1]-5,cm[2],cm[1]+5,cm[2])
          # vertical
          cmy <- c(cm[1],cm[2]-5,cm[1],cm[2]+5)
          #
          ret[[iret_]] <- df3_ %>% 
            plot_ly(
              type="scatter",
            ...
            plotly::layout(
              title=paste(
                ...
                "\n",
                ifelse(
                  argCharterOnly==TRUE,"Charters Only",""
                ),
                nschools_,
                "Schools -",
                "Cross-hairs at Center of Mass:",
                round(cm[1]),
                "/",
                round(cm[2])
              ),
...
```{r echo=FALSE}
fn_knit_exit("fn_scatterplots_hist_comp")
```  

```{r fn_scatterplots_hist_comp}  
inventory[nrow(inventory) + 1,] = list("fn_scatterplots_hist_comp","plts","","dfDisagX20_345_comp","2")
fn_scatterplots_hist_comp <- function(
  argEDTier="ALL",
  argLEAs="ALL",
  argCharterOnly=FALSE,
  argX2080excluded=FALSE  ) {
#
# complementary to fn_scatterplots_plotly_comp
# produces histograms for the scatterplot axes
#
# argEDTier is ALL or in 1:3 and links to dfLEA
# at most one of 1:3
#
    ...
    for ( LEA_ in argLEAs ) {
      ...
      for (S_ in c("MA","RD")) {
        for (G_ in c("03","04","05")) {
          ...
          #
          mean_12 <- round(mean(df3_[df3_$variable=="diff_12","value"]),digits=1)
          mean_23 <- round(mean(df3_[df3_$variable=="diff_23","value"]),digits=1)
          sd_12 <- round(sd(df3_[df3_$variable=="diff_12","value"]),digits=1)
          sd_23 <- round(sd(df3_[df3_$variable=="diff_23","value"]),digits=1)
          #
          ...
          ret[[iret_]] <- 
            ...
            labs(
              title=paste(
                ...
                "Schools",
                ifelse(
                  argX2080excluded==TRUE," Excludes Those with GLP% <20 or >80",""
                )
              ),
              subtitle=paste0(
                "Mean/sd ",xaxis_text[Years_],": ",mean_12,"/",sd_12,
                "   ",
                "Mean/sd ",yaxis_text[Years_],": ",mean_23,"/",sd_23
              ),
              x="Differences",
              fill="Years"
            ) + 
            scale_fill_discrete(
              labels=c(xaxis_text[Years_],yaxis_text[Years_])
            )
        }
      }
    }
...
track cohort: grade 3 -> 4 -> 5 across years  
```{r echo=FALSE}
fn_knit_exit("fn_scatterplots_plotly_track")
```  

use dfDisagX20_345_ABC_track and dfDisagX20_345_BCD_track

```{r fn_scatterplots_plotly_track}  
inventory[nrow(inventory) + 1,] = list("fn_scatterplots_plotly_track","plts","","dfDisagX20_345_track","2")
...
inventory[nrow(inventory) + 1,] = list("fn_dfScoreFreqNCDPI","df","score freq","score freq data","2")
...
inventory[nrow(inventory) + 1,] = list("fn_dfScoreFreqNCDPIpct","df","score freq","dfScoreFreqNCDPI","2")
...
inventory[nrow(inventory) + 1,] = list("fn_scoreFreqNCDPIBreaks","df","score freq","dfScoreFreqNCDPI","2")
...
```{r echo=FALSE}
fn_knit_exit("fn_dfScoreFreqNCDPIBreaks")
```  

```{r fn_dfScoreFreqNCDPIBreaks} 
inventory[nrow(inventory) + 1,] = list("fn_dfScoreFreqNCDPIBreaks","df","score freq","fn_scoreFreqNCDPIBreaks","2")
fn_dfScoreFreqNCDPIBreaks <- function() {
# convert list of breaks to a dataframe
  df_ <- data.frame(t(sapply(fn_scoreFreqNCDPIBreaks(),c)))
  df_ %<>%
    dplyr::mutate(
      Level2=round(X1),
      Level3=round(X2),
      ...
      Level5=round(X4),
      Max=round(X5),
      f_acYear=substr(row.names(df_),1,1),
      Grade=substr(row.names(df_),2,2),
      Subject=substr(row.names(df_),3,nchar(row.names(df_)))
     ) %>%
    dplyr::select(
      Level2,Level3,Level4,Level5,
      Max,f_acYear,Grade,Subject
    ) %>%
    dplyr::inner_join(
      dfScoreFreqNCDPI %>%
        dplyr::select(
        ...
        dplyr::ungroup(),
      by=c("f_acYear","Grade","Subject")
    ) %>%
    dplyr::rowwise() %>%
    dplyr::mutate(
      acYear=names(acYearToLETTER[which(LETTERS==f_acYear)])
    ) %>%
    data.frame() %>%
    dplyr::select(
      f_acYear,acYear,Grade,Subject,
      Min,Level2,Level3,Level4,Level5,Max
    )
    #
  return(df_)
}
#
str(fn_dfScoreFreqNCDPIBreaks())
dfScoreFreqNCDPIBreaks <- fn_dfScoreFreqNCDPIBreaks()
```  

```{r echo=FALSE}
fn_knit_exit("fn_dfScoreFreqNCDPIquartiles")
fn_knit_exit("dfScoreFreqNCDPIquartiles")
```  

Score-Freq Quartiles  

```{r fn_dfScoreFreqQuartiles}
inventory[nrow(inventory) + 1,] = list("fn_dfScoreFreqQuartiles","df","score freq","dfScoreFreqNCDPI","2")
fn_dfScoreFreqNCDPIquartiles <- function(){
# quartiles of the score-freq data
  ...
  #
  return(dfScoreFreqNCDPIquartiles)
}
str(fn_dfScoreFreqNCDPIquartiles())
dfScoreFreqNCDPIquartiles <- fn_dfScoreFreqNCDPIquartiles()
```  
...
inventory[nrow(inventory) + 1,] = list("fn_pltScoreFreqNCDPI","plts","score freq","dfScoreFreqNCDPI etc","2")
fn_pltScoreFreqNCDPI <- function(
  argCOW=FALSE,
  argSave=TRUE ) {
  # argCOW prepares plots for use with cowplot 
  # by repressing color legend
  #
  fn_p50 <- function(Y_,G_,S_){
    return(
      unlist(
        dfScoreFreqNCDPIquartiles %>%
        dplyr::filter(
  ...
  if (length(unique(dfScoreFreqNCDPI$acYear))!=3) stop("REVISE fn_pltScoreFreqNCDPI")
  #
  ret <- list()
  for (Y_ in unique(dfScoreFreqNCDPI$acYear)) {
    ...
    for (S_ in c("ELA","MA")) {
      for (G_ in 3:5) {
        ...
        geom_point() +
        geom_vline( 
          xintercept=fn_scoreFreqNCDPIBreaks()[[paste0(f_,G_,S_)]], 
          linetype='dotted' 
        ) +
        theme_bw() +
        theme( panel.grid.major.x = element_blank()) +
        theme( panel.grid.minor.x = element_blank()) +
        { if (argCOW) guides(color=FALSE) } +
        ylab("Frequency") +
        ggtitle(paste(Y_,"Grade",G_,"Subject",S_)) +
        theme(
          plot.title=element_text(size=10, hjust=0.5 ),
        ...
        annotate(
          ...
          x=-Inf,
          y=Inf,
          label=paste("Weighted Median",fn_p50(Y_,G_,S_)),
          hjust=-0.2,
          vjust=2.0,
          size=2.5
        )
...
```{r fn_plt_hist_GLPf}
inventory[nrow(inventory) + 1,] = list("fn_plt_hist_GLPf","plts","GLP histograms","dfDisagX20_345_Yr","2")
