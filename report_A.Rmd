...
**Figure I.2. ...**</div>  
<br>
Figure I.2 selects one grade (5) and one subject (Reading) across the four academic years from 2013-14 to 2016-17. The boxes are shown in the same order as above.  
...
In this section I first look more closely at the GLP% profiles for the ethnicities Black, Hispanic, and White, and alos at the EDS Yes-No categories. Figure I.4 shows these categories in histograms of GLP% for 2016-17 Reading for grade 5 over all schools.  
...
These are the schools that had four years of high GLP%, as described above:  
```{r fn_dfAnalyze_DisagX20_345_report_4, echo=FALSE}
x <- dfRes %>% dplyr::filter(nyears==4)
x
```  
<br>
These schools had three years of high GLP%:  
```{r fn_dfAnalyze_DisagX20_345_report_3, echo=FALSE}
x <- dfRes %>% dplyr::filter(nyears==3)
x
```  
<br>
These schools had two years of high GLP%:    
