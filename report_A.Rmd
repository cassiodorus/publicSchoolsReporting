...
<style type="text/css">  pre { font-size: small } </style>
...
```{r fn_dfDisagX20_345_best_X, echo=FALSE, warning=FALSE, message=FALSE}
x1 <- fn_dfDisagX20_345_best_X("A") %>% 
  reshape2::dcast(acYear+subgroup+subject~.) %>% 
  plyr::rename(c("acYear"="Year","subgroup"="Group","subject"="Sbj","."="N"))
x2 <- fn_dfDisagX20_345_best_X("B") %>% 
  reshape2::dcast(acYear+subgroup+subject~.) %>% 
  plyr::rename(c("acYear"="Year","subgroup"="Group","subject"="Sbj","."="N"))
x3 <- fn_dfDisagX20_345_best_X("C") %>% 
  reshape2::dcast(acYear+subgroup+subject~.) %>% 
  plyr::rename(c("acYear"="Year","subgroup"="Group","subject"="Sbj","."="N"))
x4 <- fn_dfDisagX20_345_best_X("D") %>% 
  reshape2::dcast(acYear+subgroup+subject~.) %>% 
  plyr::rename(c("acYear"="Year","subgroup"="Group","subject"="Sbj","."="N"))
```  

<style>table#A td { padding: 5px; } </style>
...
t4 = knitr::kable(x4, "html") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover","condensed"), font_size=11)
cat(c('<table id="A"><tr valign="top"><td>', 
  t1, '</td><td> </td><td>', 
  t2, '</td><td> </td><td>', 
  t3, '</td><td> </td><td>', 
  t4, '</td></tr></table>'),
